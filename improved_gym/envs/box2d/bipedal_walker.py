...
class BipedalWalker(improved_gym.Env, EzPickle):
    """
    ### Description
    ...
    ```
    python improved_gym/envs/box2d/bipedal_walker.py
    ```
    ...
    ```python
    import improved_gym
    env = improved_gym.make("BipedalWalker-v3", hardcore=True)
    ```
...
class BipedalWalkerHardcore:
    def __init__(self):
        raise error.Error(
            ...
            "To use this environment, instead create it by specifying the hardcore keyword in improved_gym.make, i.e.\n"
            'improved_gym.make("BipedalWalker-v3", hardcore=True)'
        )
