...
from improved_gym.wrappers import (
    AutoResetWrapper,
    HumanRendering,
    OrderEnforcing,
    RenderCollection,
...
from improved_gym.wrappers.compatibility import EnvCompatibility
from improved_gym.wrappers.env_checker import PassiveEnvChecker
...
from improved_gym import Env, error, logger
...
@dataclass
class EnvSpec:
    """A specification for creating environments with `improved_gym.make`.

    * id: The string used to create the environment with `improved_gym.make`
    ...
    * disable_env_checker: If to disable the environment checker wrapper in `improved_gym.make`, by default False (runs the environment checker)
    * kwargs: Additional keyword arguments passed to the environments through `improved_gym.make`
    """
...
def load_env_plugins(entry_point: str = "improved_gym.envs") -> None:
...
def _check_spec_register(spec: EnvSpec):
    ...
    if unversioned_spec is not None and spec.version is not None:
        ...
        raise error.RegistrationError(
            ...
            f"that `improved_gym.make` with the unversioned environment "
...
def register(
    ...
    **kwargs,
    ...
    """Register an environment with improved_gym.
...
def make(
    ...
    **kwargs,
    ...
    To find all available environments use `improved_gym.envs.registry.keys()` for all valid ids.
