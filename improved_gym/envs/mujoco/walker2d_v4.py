...
class Walker2dEnv(MujocoEnv, utils.EzPickle):
    ...
    *`ctrl_cost_weight` * sum(action<sup>2</sup>)* where *`ctrl_cost_weight`* is
    ...
    ```
    env = improved_gym.make('Walker2d-v4')
    ```

    v3 and beyond take improved_gym.make kwargs such as xml_file, ctrl_cost_weight, reset_noise_scale etc.

    ```
    env = improved_gym.make('Walker2d-v4', ctrl_cost_weight=0.1, ....)
    ```
