...
import improved_gym
from improved_gym import error, logger, spaces
...
class BaseMujocoEnv(improved_gym.Env):
...
class MujocoEnv(BaseMujocoEnv):
    ...
    def _get_viewer(
        ...
        "improved_gym.envs.mujoco.mujoco_rendering.Viewer",
        "improved_gym.envs.mujoco.mujoco_rendering.RenderContextOffscreen",
    ]:
        self.viewer = self._viewers.get(mode)
        if self.viewer is None:
            if mode == "human":
                from improved_gym.envs.mujoco.mujoco_rendering import Viewer

                self.viewer = Viewer(self.model, self.data)
            elif mode in {"rgb_array", "depth_array"}:
                from improved_gym.envs.mujoco.mujoco_rendering import RenderContextOffscreen
