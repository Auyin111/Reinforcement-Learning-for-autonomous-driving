...
from improved_gym import utils
from improved_gym.envs.mujoco import MujocoEnv
...
class HalfCheetahEnv(MujocoEnv, utils.EzPickle):
    ...
    ```
    env = improved_gym.make('HalfCheetah-v2')
    ```

    v3 and v4 take improved_gym.make kwargs such as xml_file, ctrl_cost_weight, reset_noise_scale etc.

    ```
    env = improved_gym.make('HalfCheetah-v4', ctrl_cost_weight=0.1, ....)
    ```
    ...
    * v3: support for improved_gym.make kwargs such as xml_file, ctrl_cost_weight, reset_noise_scale etc. rgb rendering comes from tracking camera (so agent does not run away from screen)
