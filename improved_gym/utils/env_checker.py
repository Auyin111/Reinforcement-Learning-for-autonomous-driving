...
import improved_gym
from improved_gym import logger, spaces
from improved_gym.utils.passive_env_checker import (
    check_action_space,
...
def check_reset_seed(env: improved_gym.Env):
    ...
    if "seed" in signature.parameters or (
        ...
        raise improved_gym.error.Error(
            "The `reset` method does not provide a `seed` or `**kwargs` keyword argument."
        )


def check_reset_options(env: improved_gym.Env):
...
def check_reset_return_info_deprecation(env: improved_gym.Env):
...
def check_seed_deprecation(env: improved_gym.Env):
    ...
    if callable(seed_fn):
        logger.warn(
            ...
            "Standard practice is to reset improved_gym environments using `env.reset(seed=<desired seed>)`"
        )


def check_reset_return_type(env: improved_gym.Env):
...
def check_env(env: improved_gym.Env, warn: bool = None, skip_render_check: bool = False):
    ...
    assert isinstance(
        env, improved_gym.Env
    ), "The environment must inherit from the improved_gym.Env class. See https://www.gymlibrary.dev/content/environment_creation/ for more info."
