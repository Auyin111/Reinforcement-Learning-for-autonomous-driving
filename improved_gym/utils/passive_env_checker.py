...
from improved_gym import Space, error, logger, spaces
...
def check_space(
    ...
    if not isinstance(space, spaces.Space):
        raise AssertionError(
            f"{space_type} space does not inherit from `improved_gym.spaces.Space`, actual type: {type(space)}"
        )
...
def env_reset_passive_checker(env, **kwargs):
    ...
    if "seed" not in signature.parameters and "kwargs" not in signature.parameters:
        logger.warn(
            "Future improved_gym versions will require that `Env.reset` can be passed a `seed` instead of using `Env.seed` for resetting the environment random number generator."
        )
    else:
    ...
    if "options" not in signature.parameters and "kwargs" not in signature.parameters:
        logger.warn(
            "Future improved_gym versions will require that `Env.reset` can be passed `options` to allow the environment initialisation to be passed additional information."
        )
