"""Utility functions for improved_gym spaces: batch space and iterator."""
...
from improved_gym.error import CustomSpaceError
from improved_gym.spaces import Box, Dict, Discrete, MultiBinary, MultiDiscrete, Space, Tuple
...
def batch_space(space: Space, n: int = 1) -> Space:
    ...
    Example::
        ...
        >>> from improved_gym.spaces import Box, Dict
        >>> space = Dict({
    ...
    """
    raise ValueError(
        f"Cannot batch space with type `{type(space)}`. The space must be a valid `improved_gym.Space` instance."
    )
...
def iterate(space: Space, items) -> Iterator:
    ...
    """
    raise ValueError(
        f"Space of type `{type(space)}` is not a valid `improved_gym.Space` instance."
    )
...
def _iterate_custom(space, items):
    raise CustomSpaceError(
        ...
        "is a custom `improved_gym.Space` instance (i.e. not one of "
        "`Box`, `Dict`, etc...)."
    )
