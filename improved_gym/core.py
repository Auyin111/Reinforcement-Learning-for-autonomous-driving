...
from improved_gym import spaces
from improved_gym.logger import warn
from improved_gym.utils import seeding

if TYPE_CHECKING:
    from improved_gym.envs.registration import EnvSpec
...
class Env(Generic[ObsType, ActType]):
    ...
    - :attr:`spec` - An environment spec that contains the information used to initialise the environment from `improved_gym.make`
    ...
    def unwrapped(self) -> "Env":
        ...
        Returns:
            Env: The base non-wrapped improved_gym.Env instance
        """
        return self
...
class ObservationWrapper(Wrapper):
    ...
    observation wrapper like this::
        ...
        class RelativePosition(improved_gym.ObservationWrapper):
...
class RewardWrapper(Wrapper):
    ...
    To do that, we could, for instance, implement the following wrapper::
        ...
        class ClipReward(improved_gym.RewardWrapper):
...
class ActionWrapper(Wrapper):
    ...
    Let’s say you have an environment with action space of type :class:`improved_gym.spaces.Box`, but you would only like
    to use a finite subset of actions. Then, you might want to implement the following wrapper::

        class DiscreteActions(improved_gym.ActionWrapper):
        ...
        if __name__ == "__main__":
            env = improved_gym.make("LunarLanderContinuous-v2")
