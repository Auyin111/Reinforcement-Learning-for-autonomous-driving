...
class GrayScaleObservation(improved_gym.ObservationWrapper):
    ...
    Example:
        ...
        >>> env = GrayScaleObservation(improved_gym.make('CarRacing-v1'))
        >>> env.observation_space
        Box(0, 255, (96, 96), uint8)
        >>> env = GrayScaleObservation(improved_gym.make('CarRacing-v1'), keep_dim=True)
        >>> env.observation_space
        Box(0, 255, (96, 96, 1), uint8)
    """

    def __init__(self, env: improved_gym.Env, keep_dim: bool = False):
        """Convert the image observation from RGB to gray scale.
