...
from improved_gym.utils.step_api_compatibility import (
    convert_to_done_step_api,
    convert_to_terminated_truncated_step_api,
)


class StepAPICompatibility(improved_gym.Wrapper):
    ...
    Examples:
        >>> env = improved_gym.make("CartPole-v1")
        >>> env # wrapper not applied by default, set to new API
        <TimeLimit<OrderEnforcing<PassiveEnvChecker<CartPoleEnv<CartPole-v1>>>>>
        >>> env = improved_gym.make("CartPole-v1", apply_api_compatibility=True) # set to old API
    ...
    def __init__(self, env: improved_gym.Env, output_truncation_bool: bool = True):
