...
class FilterObservation(improved_gym.ObservationWrapper):
    """Filter Dict observation space by the keys.

    Example:
        >>> import improved_gym
        >>> env = improved_gym.wrappers.TransformObservation(
        ...     improved_gym.make('CartPole-v1'), lambda obs: {'obs': obs, 'time': 0}
        ... )
        >>> env.observation_space = improved_gym.spaces.Dict(obs=env.observation_space, time=improved_gym.spaces.Discrete(1))
        >>> env.reset()
    ...
    def __init__(self, env: improved_gym.Env, filter_keys: Sequence[str] = None):
