...
import improved_gym
from improved_gym.error import ResetNeeded


class OrderEnforcing(improved_gym.Wrapper):
    """A wrapper that will produce an error if :meth:`step` is called before an initial :meth:`reset`.

    Example:
        >>> from improved_gym.envs.classic_control import CartPoleEnv
        >>> env = CartPoleEnv()
    ...
    def __init__(self, env: improved_gym.Env, disable_render_order_enforcing: bool = False):
        """A wrapper that will produce an error if :meth:`step` is called before an initial :meth:`reset`.
