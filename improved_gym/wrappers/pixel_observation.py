...
class PixelObservationWrapper(improved_gym.ObservationWrapper):
    ...
    Example:
        >>> import improved_gym
        >>> env = PixelObservationWrapper(improved_gym.make('CarRacing-v1', render_mode="rgb_array"))
        >>> obs = env.reset()
        >>> obs.keys()
        odict_keys(['pixels'])
        >>> obs['pixels'].shape
        (400, 600, 3)
        >>> env = PixelObservationWrapper(improved_gym.make('CarRacing-v1', render_mode="rgb_array"), pixels_only=False)
        >>> obs = env.reset()
        >>> obs.keys()
        ...
        >>> obs['pixels'].shape
        (400, 600, 3)
        >>> env = PixelObservationWrapper(improved_gym.make('CarRacing-v1', render_mode="rgb_array"), pixel_keys=('obs',))
        >>> obs = env.reset()
        >>> obs.keys()
        odict_keys(['obs'])
        >>> obs['obs'].shape
    ...
    def __init__(
        ...
        if not env.render_mode:
            raise AttributeError(
                ...
                "`improved_gym.make(env_name, render_mode='rgb_array')`."
            )
